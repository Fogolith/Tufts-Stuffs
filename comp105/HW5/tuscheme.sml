...
let
   fun ty (LITERAL(NUM x)) = inttype
     | ty (LITERAL(BOOL x)) = booltype
     | ty (LITERAL(SYM x)) = symtype
     | ty (LITERAL(PAIR (x, ys))) =  tupletype [ ty (LITERAL(x)), ty (LITERAL (ys))] 
     | ty (LITERAL(CLOSURE x))   = raise TypeError (" exception")
     | ty (LITERAL(PRIMITIVE x)) = raise TypeError (" exception")
     | ty (LITERAL (NIL)) = (FORALL(["'a"], listtype tyvarA))
     | ty (IFX(e1, e2, e3)) = 
	let 
	  val (tau1, tau2, tau3) = (ty e1, ty e2, ty e3)
          in 
          if eqType (tau1, booltype) 
	     then if eqType (tau2, tau3) 
	        then tau2
             else 
                raise TypeError ("In if expression, true branch has type " ^
                                   typeString tau2 ^ " but false branch has" ^
                                   " type " ^ typeString tau3)
          else
             raise TypeError ("Condition in if expression has type " ^
                                typeString tau1 ^ ", which should be " ^
                                typeString booltype)
          end
     | ty (APPLY(exp, actuals)) = 
	let 
	  val (tau1, actualTypes) = (ty exp, map ty actuals)
          in
	case (tau1)
	of (CONAPP (TYCON "function", [(CONAPP (TYCON "tuple",
						expectedTypes)), result])) =>
           if eqTypes (actualTypes, expectedTypes) 
	     then result
           else 
             raise TypeError ("arg types")
 
	| (FORALL (l, tau)) => raise TypeError ("some")	
     
	| _ => raise TypeError ("catchall")
	end

      | ty (SET (name, exp)) = typeof(exp, (bind (name, typeof(exp, gamma, delta), gamma)), delta)
	  ...
	  in
	      ...
	      raise TypeError ("first expression is bool, not " ^ typeString tau1)
          end 
      | ty (BEGIN (exps)) = 
          let
            val (types) = (map ty exps) 
            fun last tau [] = tau
              | last tau (h::t) = last h t
          in last unittype types
          end
      | ty (LETX(LET, alist, exp)) = 
	  let
	    val (names, types) = (map fst alist, (map ty (map snd alist)))
          in
            typeof (exp, (bindList (names, types, gamma)), delta)
          end
      | ty (LETX(LETSTAR, alist, exp)) = 
	  let
	    val (etype, names, types) = (ty exp, (map fst alist), (map snd alist)) 
	  in
            if ((null alist)) 
	      then etype 
            else
              typeof((LETX(LETSTAR, (tl alist), exp)), (bind ((hd names), ((ty (hd types))), gamma)), delta)  
          end
      | ty (LAMBDA lambda_exp) = 
	  let 
	    val (names, types) = (((map fst (fst (lambda_exp)))), (map snd (fst (lambda_exp))))  
          in (funtype (types, typeof (snd (lambda_exp), (bindList (names, types, gamma)), delta)))
	  end
      | ty (TYAPPLY (exp, tylist)) = (instantiate ((ty exp),tylist, delta))
      | ty (TYLAMBDA (tyformals, exp)) = raise TypeError ("error in TYLAMBDA")
(*	   let 
	     val (taue) = (ty exp) 
	   in
	     if (null tyformals) 
               ...
               typeof(TYLAMBDA ((tl tyformals), exp), gamma, (bind ((hd tyformals), taue, delta)))
           end
*)
  in ty e
  end
...
fun elabdef (d, gamma, delta) = 
   case d
     of
	VAL (name, exp) => 
	  ((bind (name, typeof (exp, gamma, delta), gamma)), 
                                  (typeString (typeof (exp, gamma, delta))))
      | VALREC (name, tyex, exp) => 
	  ((bind (name, tyex, gamma)), typeString (typeof (exp, gamma, delta)))
      ...
      | USE _ => raise TypeError ( "'use' reached the type checker")
