...
(*this doesn't do anything for empty array, raises out of bounds run-time*)
(*doesn't catch if a is an int or bool value*)
       | ty (AGET (a, i)) = let val (tau1, tau2) = (ty a, ty i) in
		            if eqType(tau2, INTTY) then
                                 case (tau1) of ARRAYTY(thing) => thing
                               | (_) => raise TypeError 
					("Expects (array ty) not " ^ 
                                         typeString tau1)

                            else raise TypeError ("second argument" ^ 
                                       " expects int not " ^ typeString tau2) 
		            end

      | ty (ASET (a, i, e))    = let val (tau1, tau2, tau3) = (ty a, ty i, ty e)                                 in if eqType(tau2, INTTY) then
                                     if eqType(tau1, ARRAYTY(tau3)) then tau3
                                    else raise TypeError ("Expects (array " ^ 
                                                          typeString tau3 ^ ") not " ^ typeString tau1) 
                                 else raise TypeError ("Argument 2 expects INT"
                                                   ^" not " ^ typeString(tau2)) 
                                end 


      | ty (AMAKE (len, init)) = let val (tau1, tau2) = (ty len, ty init) 
				 in if eqType (tau1, INTTY) then 
                                              ARRAYTY(tau2) else
                                              raise LeftAsExercise "error" end

      | ty (ALEN a)            = let val (taua) = (ty a) in
                                 if eqType(taua, INTTY) then raise TypeError
                                 ("Expected array not int")
				 else if eqType(taua, BOOLTY) 
                                   then raise TypeError 
                                   ("Expected array not bool")
                                 else INTTY 
                                 end
